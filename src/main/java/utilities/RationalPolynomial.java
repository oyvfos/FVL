...
public class RationalPolynomial {
	...
	private static final RationalPolynomial ONE = new RationalPolynomial(BigRational.ONE, 0);;
    ...
    public double[] getCoefd(){
        ...
        for (int i = 0; i < r.length; i++)
            r[i] = this.coef[i].doubleValue();
    	return r;
    }
    public void setCoef(BigRational[] coef){
    	this.coef =coef;
    }
    ...
    public RationalPolynomial mult(RationalPolynomial b) {
    	if (b.deg == 0) return ONE;
        RationalPolynomial a = this;
        RationalPolynomial c = new RationalPolynomial(BigRational.ZERO, a.coef.length-1);
        for (int n = 0; n <= a.deg; n++)
            for (int i = 0; i <= n; i++)
                c.coef[n] = c.coef[n].plus(a.coef[i].times(b.coef[n-i]));
        c.deg = c.degree();
        return c;
    }
